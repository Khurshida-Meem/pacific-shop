  ...
  for (const product of allProducts) {
    ...
    <div class="single-product h-100 text-center">
      ...
      <div>
        ...
        <h6>Rating: <strong>${product.rating.rate}<i class="fas fa-star "></i></strong> (${product.rating.count} Ratings)</h6>
        <h3 class=" text-success py-2">Price: $ ${product.price}</h3>
        <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success rounded-pill">add to
          cart</button>
        <button id="details-btn" class="btn btn-warning text-success fw-bold rounded-pill" >Details</button>
      </div>
    </div>
    `;
